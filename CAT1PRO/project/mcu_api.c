...
unsigned char mcu_dp_value_update(unsigned char dpid, unsigned long value)
{
    ...
    if(stop_update_flag == ENABLE)								// ENABLE:停止一切数据上传
        return SUCCESS;
    ...
    wifi_uart_write_frame(STATE_UPLOAD_CMD, MCU_TX_VER, send_len);	// 状态上报、MCU 发送帧协议版本号(默认)、
...
void uart_receive_input(unsigned char value)
{
    ...
    if(1 == rx_buf_out - rx_buf_in) {
        ...
        //若rx_buf_in已经指向buf的底部，则重新指向buf的起始地址
...
void wifi_uart_service(void)
{
    ...
    if(rx_in < PROTOCOL_HEAD)			//固定协议头长度（必须大于七个数据位）
        return;
    
    while((rx_in - offset) >= PROTOCOL_HEAD) {
        if(wifi_data_process_buf[offset + HEAD_FIRST] != FRAME_FIRST) {			//根据帧头第一个字节是0x55，确定出offset大小，即确定出帧头位置
            offset ++;
            continue;
        }
        
        if(wifi_data_process_buf[offset + HEAD_SECOND] != FRAME_SECOND) {		//帧头第二个字节0xaa
            offset ++;
            continue;
        }  
        
        if(wifi_data_process_buf[offset + PROTOCOL_VERSION] != MCU_RX_VER) {	//模块发送帧协议版本号
            offset += 2;
            continue;
        }      
        
        rx_value_len = wifi_data_process_buf[offset + LENGTH_HIGH] * 0x100;		//数据长度
        ...
        if((rx_in - offset) < rx_value_len) {	//未接收完成
            break;
        }
...
void wifi_protocol_init(void)
{
    ...
    wifi_work_state = WIFI_SATE_UNKNOW;				// 尚未配置WIFI工作模式
#endif
}
