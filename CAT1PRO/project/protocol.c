 ...
 *          2. DP(功能点)的ID/TYPE及数据处理函数都需要用户按照实际定义实现
...
void all_data_update(void)
{
    ...
    */
    mcu_dp_bool_update(DPID_SWITCH, data.Motor_State); //BOOL型数据上报;
    mcu_dp_value_update(DPID_COUNTDOWN, data.Remaining_Power); //VALUE型数据上报;
}
...
static unsigned char dp_download_countdown_handle(const unsigned char value[], unsigned short length)
{
    ...
    countdown = mcu_get_dp_download_value(value,length);		// mcu获取app下发数值
    /*
    //VALUE type data processing
    
    */
    //report_input_event(INPUT_SET_TIME, countdown);				// mcu处理新定时数值
...
static unsigned char dp_download_switch_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(switch_1 == 0) {
        //bool off
        //先不做开关功能
        //report_input_event(INPUT_POWER, switch_1);
    }else {
        //bool on
        //report_input_event(INPUT_POWER, switch_1);
    }
